document.addEventListener('DOMContentLoaded', () => {
    ...
    if (idCasilla < (width*width-width) && !estaBordeIzq) click(casillas[idCasilla-1+width]);}, 10); }
    ...
    resultado.textContent = 'Mellora para a próxima, xa que, PERDICHES!!!';
    ...
    compruebaPartida();} else { casilla.classList.remove('bandera'); casilla.innerHTML = ''; numBanderas--;}}}
    ...
    finPartida = true;
    resultado.textContent = 'Moi ben GANACHES!!!';
    ...
    if (width<6 || width>20) {
    alert(`O tamaño non pode ser menor de 6 nin maior de 20`);
    return;}
    if (numBombas<1) {
    alert(`Mínima 1 bomba personiña jeje`);
    return;}
    if (numBombas > width*width) {
    alert(`ATENCIÓN! O número de bombas non pode ser superior á multiplicación de \"Tamaño\" x \"Tamaño\" (${width*width})`);
    return;}
    if (contenedorJuego.classList.contains('hidden')) {
    contenedorJuego.classList.remove('hidden');} else {
    juego.innerHTML = "";
    resultado.innerHTML = "";
